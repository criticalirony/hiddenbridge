...
build-ver: $(GEN_SRCS) ## build the Hidden Bridge with version number
	GO111MODULE=on CGO_ENABLED=0 GOPROXY="off" $(GOBIN) build -ldflags "-X hiddenbridge/pkg/build.version=$(VERSION) -X hiddenbridge/pkg/build.buildDate=$(DATE)" -o hiddenbridge ./cmd/hiddenbridge
